...
class QueryParameterMixed_3_CheckerTest {
    ...
    public void amountALL_fromDateALL_toDateALL_queryParam_MixedDecimalNumberBooleanWholeNumberString() {
        // Since for this particular test I only want to 'use' 1 profile (and did not provide the data for a second),
        // I need to overwrite the profileList sent in the query
        profileList = new ArrayList<>(
                Arrays.asList(
                        new Profile("Jack", "./")
                )
    ...
    @Test
    public void amountALL_fromDate2017_toDate2022_queryParam_MixedWholeNumberBooleanDateString() {
        ArrayList<HashMap> queryParamsList = new ArrayList<>(
                Arrays.asList(
                        getQueryParam("Age", "LESS_THAN", 30.0),
                        getQueryParam("Has-profiletext", "EQUALS", true),
        ...
        Assertions.assertEquals(resultDetails.get(1).get("Name"), "Priscilla123-Jack2");
        Assertions.assertEquals(resultDetails.get(1).get("Age"), 23);
        Assertions.assertEquals(resultDetails.get(1).get("Has-profiletext"), true);
        Assertions.assertEquals(resultDetails.get(1).get("Vibe-conversation"), 8);
        Assertions.assertEquals(resultDetails.get(1).get("Job"), "Docente");
    ...
    public void amountLAST2_fromDate2017_toDate2022_queryParam_MixedWholeNumberBooleanDateString() {
        ...
        assertThat(raisedException).isInstanceOf(RuntimeException.class)
                .hasMessageContaining("Amount option 'LAST_xxx' cannot be set if multiple profiles are active");
